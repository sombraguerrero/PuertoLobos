...
function Affirm() {
	...
	const contentReq = https.request(contentOptions, (res) => {
	  ...
	  res.on('end', () => {
			  ...
			  {
				  var parsedData = JSON.parse(rawData);
				  myResolve(parsedData.affirmation);
			  }
			  catch (e) 
			  {
				  myReject(`${res.statusCode}: ${res.statusMessage}`);
...
function BeastPromise() {		
		return new Promise(function(myResolve, myReject) {
			var beasts = ['🐈','🐕','🐉','🐑','🦀','🐳','🦘','🐟','🦒','🦑','🐄','🦉','🪿','🦄','🦬','🐷','🐭','🦔','🦡','🦤','🦞','🐢','🐺','🐇'];
			try {
				myConsts.getInt(0,beasts.length,2)
				.then(
					function(vals)
					{
...
async function getTimePromise() {
	...
	try {
		...
		let timePromise = new Promise(async function(myResolve) {
				...
				.setColor('#0099ff');
				var rows = await conn.query("WITH `current_time` AS (SELECT NOW() AS now_utc, @@SESSION.time_zone AS session_tz) SELECT CONVERT_TZ(now_utc, session_tz, 'UTC') AS timeConverted, 'UTC' AS ZONE FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/New_York'), 'US Eastern' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Chicago'), 'US Central' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Denver'), 'US Mountain' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Phoenix'), 'US Arizona' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Los_Angeles'), 'US Pacific' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Sao_Paulo'), 'Brazil' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Europe/London'), 'UK' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Europe/Rome'), 'Central Europe' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Europe/Bucharest'), 'Eastern Europe' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Asia/Manila'), 'Philippines' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Australia/Melbourne'), 'Australia' FROM `current_time`;");
		  ...
		  switch (command.toLowerCase())
		  {
			...
			break;
			  ...
			  **/
			  ...
			  case 'affirm':
			  Affirm().then(
				function(aff) { message.channel.send(aff); },
				function(err) { message.channel.send(err); }
			  );
			  break;
