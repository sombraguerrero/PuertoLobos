...
async function getTimePromise() {
	...
	try {
		...
		let timePromise = new Promise(async function(myResolve) {
				...
				.setColor('#0099ff');
				var rows = await conn.query("WITH `current_time` AS ( SELECT NOW() AS now_utc, @@SESSION.time_zone AS session_tz ) SELECT CONVERT_TZ(now_utc, session_tz, 'UTC') AS timeConverted, 'UTC' AS zone FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Los_Angeles'), 'Pacific' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Denver'), 'Mountain' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Chicago'), 'Central' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/New_York'), 'Eastern' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'America/Sao_Paulo'), 'Brazil' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Europe/Dublin'), 'UK' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'CET'), 'Italy/Slovenia/Switzerland' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Europe/Bucharest'), 'Romania' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Asia/Manila'), 'Philippines' FROM `current_time` UNION ALL SELECT CONVERT_TZ(now_utc, session_tz, 'Australia/Melbourne'), 'Australia' FROM `current_time`;");
